...
## Real PDF Generation with wkhtmltopdf (Jan 9, 2026) - Issue #9
- [ ] Update pdfGenerator to use wkhtmltopdf instead of html-pdf-node
- [ ] Generate real PDF with Content-Type: application/pdf
- [ ] Filename: cards-xxxx.pdf (no .html extension)
- [ ] Add Cache for repeated batches (same batch + template + settings = same PDF)
- [ ] Add Retry logic (1 retry on failure)
- [ ] Add Loading indicator in frontend
- [ ] Ensure PDF matches preview exactly (fonts, positions, colors)
- [ ] QR Code: OFF by default, size 50, X/Y within card bounds
